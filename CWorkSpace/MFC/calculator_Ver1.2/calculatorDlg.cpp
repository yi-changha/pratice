...
void CcalculatorDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_1, m_edit_1);
	DDX_Text(pDX, IDC_EDIT_2, m_edit_2);
	DDX_Text(pDX, IDC_EDIT_3, m_edit_3);
	DDX_Text(pDX, IDC_EDIT_4, m_edit_4);
	DDX_Text(pDX, IDC_EDIT_5, m_edit_5);
}
...
{
	int n_1 = 0, n_2 = 0, n_3 = 0, n_4 = 0, n_5 = 0;
	...
	// Bubble Sort(거품 정렬)
	void sort_lotto(int arr[], int sort_n);
	{
		int temp = 0;
		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 6; j++) {
	...
	for (int i = 0; i < 6; i++) {
		str.Format(_T("%2d "), arr[i]); // 숫자 6개 변수에 기억

		if (arr[i] >= 1 && arr[i] <= 9) n_1++;
		if (arr[i] >= 10 && arr[i] <= 19) n_2++;
		if (arr[i] >= 20 && arr[i] <= 29) n_3++;
		if (arr[i] >= 30 && arr[i] <= 39) n_4++;
		if (arr[i] >= 40 && arr[i] <= 45) n_5++;
	...
	// 번호 구간별 개수 출력
	m_edit_1.Format(_T("%d"), n_1); // 1 ~ 9 사이 갯수
	m_edit_2.Format(_T("%d"), n_2); // 10 ~ 19
	m_edit_3.Format(_T("%d"), n_3); // 20 ~ 29
	m_edit_4.Format(_T("%d"), n_4); // 30 ~ 39
	m_edit_5.Format(_T("%d"), n_5); // 40 ~45

	UpdateData(false); // 변수 => 화면에 출력
}
