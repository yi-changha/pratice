...
void CScrollBarView::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_SCROLLBAR_H, m_hsb);
	DDX_Control(pDX, IDC_SCROLLBAR_V, m_vsb);
}
...
void CScrollBarView::OnDraw(CDC* pDC)
{
	...
	CFont font;
	LOGFONT lf = { 0 }; // 구조체를 0으로 초기화 한다.
	lf.lfHeight = vpos / 3 + 1; // 폰트 크기를 설정한다. vpos == 0 이면 크기는 1.
