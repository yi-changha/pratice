...
BEGIN_MESSAGE_MAP(CListBoxView, CFormView)
	...
	ON_BN_CLICKED(IDC_BUTTON_RIGHT, &CListBoxView::OnBnClickedButtonRight)
	ON_BN_CLICKED(IDC_BUTTON_LEFT, &CListBoxView::OnBnClickedButtonLeft)
END_MESSAGE_MAP()
...
void CListBoxView::OnBnClickedButtonRight()
{
	...
	if (nIndex != LB_ERR) {
		...
		AfxGetMainWnd()->SetWindowText(_T("오른쪽으로 이동"));
	}
}


void CListBoxView::OnBnClickedButtonLeft()
{
	...
	if (nIndex != LB_ERR) {
		...
		AfxGetMainWnd()->SetWindowText(_T("왼쪽으로 이동"));
	}
}
