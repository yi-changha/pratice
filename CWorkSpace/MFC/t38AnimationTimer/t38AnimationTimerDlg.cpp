...
void Ct38AnimationTimerDlg::OnPaint()
{
	...
	CBitmap bk1, bk2, octo1, turtle1, shark1, * pold;
	CDC memDC;
	...
	pDC->StretchBlt(image1.x, 150, 100, 75, &memDC, 0, 0, 100, 75, SRCCOPY); // image1.x timer에 의해 발생된 값
	DeleteObject(shark1);

	// 문어 이동
	pold = memDC.SelectObject(&octo1);
	pDC->StretchBlt(1920 - image1.x, 450, 100, 66, &memDC, 0, 0, 100, 66, SRCCOPY); 
	DeleteObject(octo1);

	// 거북이 이동
	pold = memDC.SelectObject(&turtle1);
	pDC->StretchBlt(1920 - image1.x, 850, 100, 72, &memDC, 0, 0, 100, 72, SRCCOPY); 
...
void Ct38AnimationTimerDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	case(2): // 타이머 2 선택 (물고기 확대)
	{
		CClientDC dc(this);
		CDC memDC;
		static CBitmap img1[3];
		static int flag = 0;
		static int lx = width1 / 100, ly = height1 / 100;
		int x = 0, y = 0;
		
		if (flag == 0) {
			img1[0].LoadBitmap(IDB_OCTO1);
			img1[1].LoadBitmap(IDB_SHARK1);
			img1[2].LoadBitmap(IDB_TURTLE1);
			flag = 1;
		}
